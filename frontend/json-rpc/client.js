...
export namespace WalletModel {
  ...
  export type <%= _.camelCase(method.name).replace('admin', '').replace('session', 'Session') %>Response = <%= methodTypings.getTypingNames('typescript', method).result %>
  <% }); %>
}
...
export class WalletClient {
   ...
   */
  // tslint:disable-next-line:max-line-length
  public <%= _.camelCase(method.name).replace('admin', '').replace('session', 'Session') %>: <%= methodTypings.getTypingNames('typescript', method).method %> = (...params) => {
  ...
  afterCompileTemplate: [
    async dest => {
      await copy(
        ...
        path.join(dest.replace('/custom/typescript', ''), 'index.ts')
      )
