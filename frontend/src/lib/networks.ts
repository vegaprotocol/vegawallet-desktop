...
export const networkPresetSchema = z.object({
  name: z.string(),
  configFileUrl: z.string()
    .url({ message: 'Invalid network configuration url.' })
    .endsWith('.toml', { message: 'Invalid network configuration file extension. Only ".toml" files are supported.' }),
})
...
export const fetchNetworkPreset = (url: string, logger: log.Logger) => fetch(url)
  .then(res => res.json())
  .then(r => {
