...
describe('manage networks', () => {
  ...
  it('edit network details displayed', function () {
    ...
    cy.contains('gRPC Nodes').next().within(() => {
      cy.getByTestId('node-list').first().invoke('val').should('not.be.empty')
    })
    cy.contains('GraphQL Nodes').next().within(() => {
      cy.getByTestId('node-list').first().invoke('val').should('not.be.empty')
    })
...
describe('change network details', () => {
  ...
  it('able to change service host', () => {
    ...
    cy.submit_and_revisit_network_config_form()
  ...
  it('able to change gRPC nodes', function () {
    cy.contains('gRPC Nodes').next().within(() => {
      ...
      cy.getByTestId('node-list').first()
        .its('val').as('first_grpc_node')
  ...
  it.skip('able to add a new REST Node', () => {
    ...
    cy.contains('REST Nodes').next().within(() => {
  ...
  Cypress.Commands.add('submit_and_revisit_network_config_form', () => {
    ...
    cy.getByTestId('manage-networks').click()
    cy.getByTestId('edit-network-test').first().click()
    cy.getByTestId('service-host', {timeout:10000}).should('be.visible')
  })
})