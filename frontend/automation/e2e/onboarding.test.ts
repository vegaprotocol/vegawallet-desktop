...
test.describe('onboarding', () => {
  ...
  test('mainnet should be selctable as default network when envvar is mainnet or empty', async () => {
  ...
  test('fairground should be selctable as default network when envvar is fairground', async () => {
    // 0001-WALL-009 - must have Mainnet and Fairground (testnet) pre-configured (with Mainnet being the default network)
    if (process.env.VITE_FEATURE_MODE === 'fairground') {
      await networkTab.openNetworkTabAndViewNetworks()
